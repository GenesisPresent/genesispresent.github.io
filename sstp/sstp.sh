...
thvpn="genesispresent.github.io/sstp"
MYIP=$(wget -qO- ipinfo.io/ip);
...
touch /var/lib/thvpn/data-user-sstp
country=ID
state=Thailand
locality=Thailand
organization=thvpn
...
wget -O /etc/accel-ppp.conf "https://${thvpn}/accel.conf"
...
#input perintah sstp
wget -O /usr/bin/addsstp https://${thvpn}/addsstp.sh && chmod +x /usr/bin/addsstp
wget -O /usr/bin/delsstp https://${thvpn}/delsstp.sh && chmod +x /usr/bin/delsstp
wget -O /usr/bin/ceksstp https://${thvpn}/ceksstp.sh && chmod +x /usr/bin/ceksstp
wget -O /usr/bin/renewsstp https://${thvpn}/renewsstp.sh && chmod +x /usr/bin/renewsstp
rm -f /root/sstp.sh
