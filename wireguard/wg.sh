...
thvpn="genesispresent.github.io/wireguard"
...
if [[ -e /etc/wireguard/params ]]; then
	echo -e "${Info} WireGuard ติดตั้งแล้ว โปรดพิมพ์ addwg เพื่อเพิ่มผู้ใช้งาน     "
	exit 1
fi

echo -e "${Info} Wireguard Script TH-VPN"
...
cd /usr/bin
wget -O addwg "https://${thvpn}/addwg.sh"
wget -O delwg "https://${thvpn}/delwg.sh"
wget -O renewwg "https://${thvpn}/renewwg.sh"
chmod +x addwg
chmod +x delwg
chmod +x renewwg
cd
